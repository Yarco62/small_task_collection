...
def make_ssh_gitpull(command = ''):
    s = pxssh.pxssh()
    if not s.login (IP_ADDR, USER, PASSWORD):
...
def make_gunic_restart_main(command = ''):
    s = pxssh.pxssh()
    if not s.login (IP_ADDR, USER, PASSWORD):
        print("SSH session failed on login.")
        print(str(s))
...
def make_gunic_restart_dev(command = ''):
    s = pxssh.pxssh()
    if not s.login (IP_ADDR, USER, PASSWORD):
        print("SSH session failed on login.")
        print(str(s))
...
if __name__ == "__main__":
    ...
    while True:
        ...
        if command[0] == 'пулловер' or command[0] == 'пуловер':
            if command[1] == "привет":
                ...
                make_ssh_gitpull('git pull origin dev')
                play_voice_assistant_speech("я сделал гит пул на дэве, хозяин")
            elif voice_input_raw.find('перезапусти') != 0:
                make_gunic_restart_dev('service gunicorn restart')
        ...
        if voice_input_raw.find('блин') != -1 or voice_input_raw.find('жесть') != -1 or voice_input_raw.find('fuck'):
